...
void formStandardFormLp(const HighsLp& lp,
  ...
  standard_form_offset_ = sense * lp.offset_;
  for (HighsInt iCol = 0; iCol < lp.num_col_; iCol++)
    standard_form_cost_.push_back(sense * lp.col_cost_[iCol]);
  standard_form_matrix_.format_ = MatrixFormat::kRowwise;
  standard_form_matrix_.num_col_ = lp.num_col_;
  ...
  for (HighsInt iRow = 0; iRow < lp.num_row_; iRow++) {
    ...
    if (lower == upper) {
      ...
      standard_form_row = standard_form_rhs_.size();
      slack_ix.push_back(standard_form_row + 1);
      standard_form_matrix_.addRows(local_row);
      standard_form_rhs_.push_back(upper);
    }
  ...
  for (HighsInt iCol = 0; iCol < lp.num_col_; iCol++) {
    ...
    if (lower > -kHighsInf && upper < kHighsInf) {
      ...
      //
      // x = u - s => x + s = u
      standard_form_cost_.push_back(0);
      standard_form_matrix_.num_col_++;
      local_row.num_col_++;
      local_row.index_[0] = iCol;
      local_row.index_[1] = standard_form_matrix_.num_col_ - 1;
  ...
  // Finished with both matrices, row-wise.
  //
  // Ensure that the standard form matrix is col-wise so RHS shifts
  // can be applied and more columns can be added
  standard_form_matrix_.ensureColwise();
  ...
  for (HighsInt iCol = 0; iCol < lp.num_col_; iCol++) {
    ...
    if (lower > -kHighsInf) {
      ...
      standard_form_offset_ += cost * upper;
      standard_form_cost_[iCol] = -cost;
      ...
      standard_form_cost_.push_back(-cost);
      for (HighsInt iEl = standard_form_matrix_.start_[iCol];
           iEl < standard_form_matrix_.start_[iCol + 1]; iEl++) {
        standard_form_matrix_.index_.push_back(
            standard_form_matrix_.index_[iEl]);
        standard_form_matrix_.value_.push_back(
            -standard_form_matrix_.value_[iEl]);
      }
      standard_form_matrix_.start_.push_back(
          HighsInt(standard_form_matrix_.index_.size()));
    }
  }
  ...
  for (HighsInt iX = 0; iX < HighsInt(slack_ix.size()); iX++) {
    ...
    standard_form_cost_.push_back(0);
    if (iRow > 0) {
      standard_form_matrix_.index_.push_back(iRow - 1);
      standard_form_matrix_.value_.push_back(1);
    } else {
      standard_form_matrix_.index_.push_back(-iRow - 1);
      standard_form_matrix_.value_.push_back(-1);
    }
